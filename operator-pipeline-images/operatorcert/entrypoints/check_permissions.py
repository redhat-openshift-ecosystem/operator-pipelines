...
class OperatorReview:
    ...
    @property
    def github_pr_number(self) -> int:
        """
        The number of the pull request

        Returns:
            int: The number of the pull request
        """
        return int(urllib.parse.urlparse(self.pull_request_url).path.split("/")[-1])
    ...
    def pr_labels(self) -> set[str]:
        ...
        return {
            x.name
            for x in self.github_client.get_repo(self.github_repo_name)
            .get_pull(self.github_pr_number)
            .get_labels()
        }
    ...
    def check_permissions(self) -> bool:
        ...
        # In case a PR is raised for catalog promotionm,
        # we will ignore the admin/write permissions check
        # and let users to review the PR manually
        is_catalog_promotion_pr = self.is_catalog_promotion_pr()
        if not is_catalog_promotion_pr and self.is_org_member():
        ...
        return not is_catalog_promotion_pr and self.pr_owner_can_write()

    def is_catalog_promotion_pr(self) -> bool:
        """
        Check if the pull request is automatically generated by the catalog promotion
        process. The PR is generated by the service account or admin of the organization
        and in such a case the PR would be approved automatically.

        For this kind of PRs we will ignore the permission check for admins and
        let users to review the PR manually.

        The logic depends on available labels in the PR. The label
        `catalog-promotion/review-needed` is added to the PR when the PR is created.

        Returns:
            bool: A boolean value indicating if the pull request is a catalog promotion PR
            that needs to be reviewed by users
        """

        try:
            pr_no = int(
                urllib.parse.urlparse(self.pull_request_url).path.split("/")[-1]
            )
            labels = {
                x.name
                for x in self.github_client.get_repo(self.github_repo_name)
            ...
            is_catalog_promotion_pr = "catalog-promotion/review-needed" in labels
            if is_catalog_promotion_pr:
                LOGGER.info(
                    "Pull request promotes operator to the next catalog version. "
                    "Ignoring permission check for admins."
                )
            return is_catalog_promotion_pr
    ...
    def is_org_member(self) -> bool:
        ...
        try:
            members = self.github_client.get_organization(
