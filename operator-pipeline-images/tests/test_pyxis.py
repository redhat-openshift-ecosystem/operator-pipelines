...
@patch("operatorcert.pyxis.time.sleep")
@patch("operatorcert.pyxis.get")
def test_wait_for_container_grades(mock_get: MagicMock, mock_sleep: MagicMock) -> None:
    mock_get.return_value.json.side_effect = [
        {},
        {"freshness_grades": [{"grade": "A"}]},
    ]
    result = pyxis.wait_for_container_grades("https://foo.com/", "123")

    assert result == {"freshness_grades": [{"grade": "A"}]}
    assert mock_get.call_count == 2
    assert mock_sleep.call_count == 1
...
@patch("operatorcert.pyxis.time.sleep")
@patch("operatorcert.pyxis.get")
def test_wait_for_container_grades_timeout(
    mock_get: MagicMock, mock_sleep: MagicMock, mock_now: MagicMock
) -> None:
    ...
    # Timeout is less than difference between current time and start time
    result = pyxis.wait_for_container_grades("https://foo.com/", "123", timeout=5)
...
@patch("operatorcert.pyxis.get")
def test_wait_for_container_grades_error(
    mock_get: MagicMock,
) -> None:
    response = Response()
    response.status_code = 400
    mock_get.return_value.raise_for_status.side_effect = HTTPError(response=response)
    with pytest.raises(HTTPError):
        pyxis.wait_for_container_grades("https://foo.com/v1", "123")
